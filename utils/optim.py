...
from horovod.torch import DistributedOptimizer, _DistributedOptimizer, Compression
        ...
        if 'optimizer' in setting:
            ...
            if not isinstance(self.optimizer, optim_method):
                ...
                logging.debug('OPTIMIZER - setting method = %s' %
                              setting['optimizer'])
        if 'horovod' in setting and setting['horovod']:
            # had to use protected member otherwise we would need to use flag.
            if not self.dist_optim_set:
                self.optimizer = DistributedOptimizer(self.optimizer,
                                                      named_parameters=self.regularizer._model.named_parameters(),
                                                      compression=Compression.none)
                self.dist_optim_set = True
